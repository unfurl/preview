...
export class PreviewService {
  ...
  private unfurlUrl = `https://unfurl.io/api/v2/preview?url=`;
  load(url: string): Observable<Preview> {
    const apiToken = this.config
      ? this.config.apiToken
      : unfurl.getConfig()?.apiToken ?? '';

    if (!apiToken) {
      console.error(
        `Unfurl's Api Token is missing please make sure you add the Api Token Appropriately as per the documentation at https://github.com/unfurl/preview`
      );
    }
    this.apiToken = apiToken;

    return this.http
      .get<UnfurlMetadata>(`${this.unfurlUrl}${url}`, {
        headers: {
